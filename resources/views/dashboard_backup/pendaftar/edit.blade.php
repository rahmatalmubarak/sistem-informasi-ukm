                    ...
                    <form method="POST" action="{{ route('pendaftar.update', $pendaftar->id)}}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('jenis_kelamin') is-invalid @enderror" name="jenis_kelamin" id="jenis_kelamin">
                                <option>Pilih</option>
                                <option value="laki-laki" @if (old('jenis_kelamin', $pendaftar->jenis_kelamin)=='laki-laki' ) {{'selected'}} @endif>Laki-laki</option>
                                <option value="perempuan" @if (old('jenis_kelamin', $pendaftar->jenis_kelamin)=='perempuan' ) {{'selected'}} @endif>Perempuan</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="tgl_lahir" class="form-control-label">Tanggal Lahir</label>
                            <input class="form-control @error('tgl_lahir') is-invalid @enderror" type="date" value="{{old('tgl_lahir', $pendaftar->tgl_lahir)}}"
                                name="tgl_lahir" id="tgl_lahir">
                        </div>
                        <div class="form-group">
                            <label for="alamat">Alamat</label>
                            <textarea class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat"
                                rows="3">{{old('alamat', $pendaftar->alamat)}}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="fakultas">Fakultas</label>
                            <select class="form-control @error('fakultas') is-invalid @enderror" name="fakultas" id="fakultas">
                                <option>Pilih</option>
                                <option value="sistem informasi" @if (old('fakultas', $pendaftar->fakultas) =='sistem informasi' ) {{'selected'}} @endif>Sistem
                                    Informasi</option>
                                <option value="bimbingan konseling" @if (old('fakultas', $pendaftar->fakultas) =='bimbingan konseling' ) {{'selected'}} @endif>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('jurusan') is-invalid @enderror" name="jurusan" id="jurusan">
                                <option>Pilih</option>
                                <option value="sistem informasi" @if (old('jurusan', $pendaftar->jurusan)=='sistem informasi' ) {{'selected'}} @endif>Sistem
                                    Informasi
                                </option>
                                <option value="bimbingan konseling" @if (old('jurusan', $pendaftar->jurusan)=='bimbingan konseling' ) {{'selected'}} @endif>
                                    Bimbingan
                                    Konseling</option>
                            </select>
                        </div>
